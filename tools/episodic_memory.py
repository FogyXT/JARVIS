...
PROMOTE_THRESHOLD = 0.65  # score above this → promote to semantic
...
_global_embed_dim = 384


def _get_global_embed_fn(dim: int = 384):
    ...
    try:
        ...
        _global_embed_fn = SentenceTransformer("all-MiniLM-L6-v2")
        log.info("Embedding model loaded (global singleton)", module="episodic",
                data={"model": "all-MiniLM-L6-v2", "dim": dim})
    except ImportError:
...
class EpisodicBuffer:
    ...
    def _decay_score(self, item: MemoryItem, delta_t: float) -> float:
        return (item.access_count ** self.beta) * math.exp(-self.lambd * delta_t) * item.importance
    ...
    def get_promotable(self) -> list[MemoryItem]:
        ...
        if promotable:
            ...
            log.info(f"Promoting {len(promotable)} memories to semantic store", module="episodic")
        return promotable
